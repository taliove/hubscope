...
- [x] 模型同时带 capability 与 family 两个分类,新增/发现/手动添加都按规则识别
- [x] 默认规则首次启动种入数据库,覆盖常见厂商与能力关键词
- [x] 管理页表格化增删改规则,保存后立即全量重算存量模型分类
- [x] 无法识别的模型归为 other,不出错、不阻塞同步
- [x] 黑盒测试:stub Hub 返回多样模型 ID,经 API 断言双维度分类正确;改规则后重算生效
