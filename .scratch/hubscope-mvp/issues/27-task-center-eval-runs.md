...
- [x] tasks 与 task_logs 随迁移存在,状态机统一(pending/running/success/failed)
- [x] 手动与每周触发的 Eval Run 都注册为 Task,状态随执行流转到 success/failed
- [x] 执行日志逐行可查:至少含每 Case 完成与裁判失败事件
- [x] 任务中心页:列表、过滤、日志查看;写操作/管理动作仍需登录,读遵循现有读权限分级
- [x] 黑盒测试:触发 run 后任务出现、日志逐行完整、失败 run 的 Task 为 failed
