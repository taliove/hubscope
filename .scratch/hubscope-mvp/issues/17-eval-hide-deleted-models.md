...
- [ ] GET /api/evals/latest 不返回已删除模型的行,现役模型不受影响
- [ ] 历史 eval run 详情(GET /api/evals/{id})仍可读,结果中 model_id 文本正常渲染
- [ ] 前端对比视图无已删除模型;历史/趋势处已删除模型带「已删除」标记
- [ ] 黑盒测试:跑评估 → 删除模型 → latest 不含该模型而 run 详情仍可读
