...
- [ ] 状态机四条规则正确,且每个状态附带人类可读的判定依据
- [ ] 总览 API 一次返回全部 Endpoint 的状态与 24h 摘要(成功率、P50、P95)
- [ ] Dashboard 状态矩阵渲染红黄绿,支持模型名/协议/状态过滤
- [ ] 黑盒测试:用 stub Hub 控制成败序列,推进假时钟,经 API 断言绿→黄→红→恢复的状态迁移与依据文案
