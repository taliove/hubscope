...
- [x] 详情 API 返回曲线数据(按时间桶)、成功率趋势与近期失败列表
- [x] 页面渲染延迟/TTFT 曲线并可切换流式/非流式
- [x] 失败记录展示 HTTP 码与错误摘要
- [x] rollup 每日生成且旧原始数据被清理;rollup 后历史曲线仍可查
- [x] 假时钟测试:跨天推进后断言 rollup 落库、超期原始记录被删、曲线 API 仍返回 rollup 数据
