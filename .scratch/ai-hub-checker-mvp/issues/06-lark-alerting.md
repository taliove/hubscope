...
- [ ] 连续 3 次失败触发一次 down 告警,内容含模型/协议/错误摘要
- [ ] 由红转绿触发一次 recovered 通知
- [ ] 同一故障持续期间不重复发送 down 告警
- [ ] webhook 地址与开关可在设置页修改并即时生效;webhook 未配置时跳过发送不报错
- [ ] 测试用 stub webhook server 断言:消息内容、发送时机、不重复发送、事件落库
