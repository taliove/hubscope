  ...
  <el-card shadow="never" class="library-card">
    ...
    <el-collapse>
      <el-collapse-item v-for="suite in suites" :key="suite.id" :name="suite.id">
        <template #title>
          <span class="suite-title">{{ suite.name }}({{ suite.key }},{{ suite.cases.length }} 题)</span>
        </template>
...
import type { EvalCase, Suite, VerdictType } from '@/api/types'
...
// Persist the form; the server validates the whole merged case.
