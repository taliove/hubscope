...
**Progress Grid**:
进度网格,Campaign 运行中的 模型 × 能力点 状态矩阵(等待中/运行中/已完成/失败),是批次进度的默认视图;半成品榜单从网格下钻查看,分数带 Coverage 水印。名次口径:半成品总分非 null 的行显示弱化实时名次(GH #40 裁决,ui-guidelines §5 + 附录 B 第 13 项;本表早期摘要「运行中不排名次」以此裁决为准)。
_Avoid_: 进度条、任务矩阵

**Jury**:
裁判团，judge 型 Case 的判分主体：按策略（balanced/speed/iq/cost）从被评模型所在 Hub 选出的 3 个裁判模型，各自独立打分，样本分取中位数（3→中位、2→均值、1→自身、0→不计分）；备选 ≥3 时被评模型被踢出自己的裁判团。Run 记录裁判团快照。
_Avoid_: 评审团、投票组

**Probe Gate**:
刺探门控，评估触发时的前置实测：对候选模型做 k 轮小调用得到通断/成功率/TPS，被评模型不通则跳过不烧题；结果只进 Task Log，不进监控探测表、不驱动状态机。
_Avoid_: 预检（状态板缓存检查是另一机制）

**Model Registry**:
模型登记表，内置的模型特性与定价静态表（IQ 档、输入/输出价 USD/1M tokens），settings 覆盖项优先；供裁判团选择与评估成本估算复用。未登记模型 IQ/价格记 nil，成本显示「价格未登记」。
_Avoid_: 价格表、模型库
