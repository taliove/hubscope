...
5. **TDD:** 新行为先写(或同批写)黑盒测试,测试走唯一接缝——HTTP API 层(stub Hub + 假时钟 + 真 SQLite 临时库),不 mock 内部模块,不断言内部状态。
   **禁 sleep 例外形态登记(GH #31 check 沉淀):** 测试禁用真实 sleep 等待时序,一律走假时钟(W4)。唯一合法例外 = 真实 HTTP 边界上的超时分派验证(先例 `internal/hubclient/timeout_test.go`),允许条件三项缺一即不合规:真实 httptest stub(非 mock)、单向裕量断言(小预算必超时/大预算必活,裕量一个数量级以上)、文件头登记为何 W1 接缝无法覆盖。
