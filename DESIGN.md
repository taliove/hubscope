...
- 状态 3+1:稳定 / 降级 / 异常 + 未验证(参照稿词表,GH #128;「未验证」= Ping 监测端点的中性第四呈现,2026-08-02 裁决 GH #160),小圆点 + 状态词双编码,局部强调而非大面积色块
- 动效成体系(v2.0 §15):数字补间、图表入场绘制、页面 Fade+Slight Move、hover 上浮;reduced-motion 全局归零

## Colors

调色板 = 「一族蓝 + 一族零彩度中性灰 + 三功能色文字/图形双阶」:品牌蓝承担交互,中性灰承担全部中性表达,功能三色承担三态语义。**不存在第四色**——告警橙随三态合并整体退役(ADR 0015);`--hs-status-failing` 令牌值为 danger 红,仅为混合期兼容保留,新世界零消费,待物理删除。**「未验证」第四呈现不是第四色相(2026-08-02,GH #160):** unverified 显示态消费中性灰族(词 placeholder 阶、点 info 灰),不引入新功能色,本条不变。
...
- **Info**(#6E6E73,gray-600):中性提示;**兼「未验证」状态点**(unverified 图形档,GH #160——词取 `--hs-text-placeholder` gray-400,点取本值;第四呈现走中性灰族,不占功能三色槽)。
...
- 全站唯一的 endpoint 状态展示组件:**小圆点(图形阶)+ 状态词(文字阶)** 双编码,3+1(稳定/降级/异常 + 未验证——未验证档 = 点 info 灰、词 placeholder 阶,GH #160);词与色槽全部来自显示层映射 `utils/statusDisplay.ts`,组件内禁写状态词字面量;degraded 可挂成因副标签(「· 可用性」「· 延迟」,secondary,不随词着色);零动画(闪烁退役)。规格与消费纪律见 ui-guidelines §5 与 overview surface brief。
