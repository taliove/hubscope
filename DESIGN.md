...
**Mono Font:** 系统等宽字栈(`ui-monospace, "SF Mono", "Cascadia Mono", Consolas, monospace`)——**品牌字族之一(2026-07-31 补登,用户批准)**:用于 Wordmark 字标(字重 700,ui-guidelines §2b 互指)、版本号(PublicFooter `.version`,GH #90 自 AppHeader 迁入)与数据等宽语境;与 Body Font 同属「零外部字体文件、离线可用」纪律(单二进制交付一致)。本登记即该字栈的设计系统出处——impeccable detector 的 design-system-font 检查以本条为准,此字栈不再视为偏离。
...
**响应式断点(2026-07-31 登记,GH #91 批):** 全站第一断点 **768px**(`(max-width: 767px)` 生效),消费方封闭清单 = 分享弹窗(StatusShareDialog/EvalShareDialog)、Leaderboard、EvalProgressGrid、/report/:token 页;状态板与管理台不做窄屏适配,「桌面优先」声明不失效。**窄屏处置原则 = 形态切换,不是横向滚动豁免**——无横滚纪律在窄屏下以换形态(弹窗物料等比缩放、榜单矩阵降级卡片式列表)守住,§4 不开豁免;断点判定统一 matchMedia 组件级实现,禁各处自造第二套断点逻辑。规格与级联算术见 share-materials surface brief。
