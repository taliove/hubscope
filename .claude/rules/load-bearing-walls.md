...
改错代价极高、撑起系统语义的结构与约定。修改任何一项前,必须按 [CLAUDE.md](../../CLAUDE.md)「承重墙」一节回答四问(为什么必须改 / 影响哪些调用方 / 有无替代方案 / 回归测试什么),并附 ADR。
...
- **约定:** Hub token 入库脱敏(省略号+后 4 位)、任何接口不回明文;管理员口令只经 `ADMIN_PASSWORD` 环境变量注入;凭证扫描门禁 fail-closed;Lark webhook 不进日志。
- **为何承重:** 泄露即事故,不可逆。
