...
**inline 修的规范回写连带检查(GH #122 check HIGH-1/MEDIUM-1 沉淀,2026-08-01 登记):** 规范文档已由 write agent 写成、而 main 或他人在 commit 前再 inline 改代码(裁决落地/小修)时,check 必须把「被 inline 修触碰的断言」在规范文档中 grep 复核——文档写于裁决前、代码落于 commit 前,同一 commit 内规范与代码会直接矛盾(首例:T13 的 h1 档与 family 词表两处)。
...
**定量 AC 的可达性预验(GH #72 check 沉淀,2026-07-30 登记):** UI 票验收标准含定量体验指标(如「卡高降 20–25%」)时,plan 回写 brief 阶段必须用 CSS 算术预验该数字可达;AC 数字与 brief 规格脱节时以 brief 为准并在票面注明(首例:卡高 AC 20–25% vs 规格算术 11–12%,check 阶段才发现口径冲突)。**推广(2026-07-31,GH #88 check MEDIUM-1 沉淀):** 凡登记「构造性相等/逐像素同规格/同宽推导」类算术(无论 AC 预验还是批末回写),必须列出完整级联逐项取值(含未被本项目覆盖的框架默认值与布局 gap),check 按级联清单逐项复算(首例:GH #88「246 = 272 − 26」漏算 EP 卡 padding 40px 与 label gap 6px;第二例:GH #133「1104 内容宽」用错 padding 档)。宽度级联算术必须先声明盒模型事实(本仓无全局 box-sizing reset,content-box)与所经每一层 padding 令牌档位,再列算式。
**编号重排类合并的机械校验(2026-07-30 支系合批 check 沉淀):** 并行支系各自对同一编号序列(如附录 B)续号后合并,冲突解决必须重排编号并**全仓 grep 所有「第 N 项 / #N」编号引用逐一对照新编号表**(首例:附录 B 双支系 14/15 撞号,解决时漏改 §7 一处「第 15 项」引用被抓为 HIGH);重排依据与引用同步清单在合并处留注记。**推广(2026-07-31,GH #85 check LOW-1 沉淀):** 同纪律从编号重排推广到一切构成/位置/命名变更——回写规范时必须 grep 旧定位词全仓核对交叉引用(首例:GH #85「本组」指标从组头行下移组条行后,ui-guidelines 三处 + surface brief 一处「组头/标头『均延』」旧定位词残留,check 抓为 LOW)。「全仓」核对含三处规范正文(DESIGN.md / surface briefs / ui-guidelines)**+ PRODUCT.md(词表/产品真相类变更——显示层词表寄存于 ui-guidelines §3.1/§7、DESIGN.md、briefs、PRODUCT.md 四处;2026-08-02 外壳抽屉/裁决批 check MEDIUM-1:三处规范同步「稳定」词而 PRODUCT.md 漏改,沉淀升级本条);同文件多节引用同一事实时逐节核对**(GH #148 check MEDIUM-2:Layout 节外壳行与 Navigation 节 AppTopbar 行同载「管理登录」,回写只改一节)——移除/迁移/**删除**具名消费方(类名/组件名)时 grep 该名称于三处规范,并核对 surface brief related_targets 的实体存活性(文件已删而清单残留即失实;第二例:GH #136 UptimeMicroStrip 删除后清单残留,check 沉淀)(首例:GH #90,同日新登的 mono 字族消费方指针 AppHeader .version 当日被迁移失效,check MEDIUM-1)。
