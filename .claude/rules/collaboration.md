...
**编号重排类合并的机械校验(2026-07-30 支系合批 check 沉淀):** 并行支系各自对同一编号序列(如附录 B)续号后合并,冲突解决必须重排编号并**全仓 grep 所有「第 N 项 / #N」编号引用逐一对照新编号表**(首例:附录 B 双支系 14/15 撞号,解决时漏改 §7 一处「第 15 项」引用被抓为 HIGH);重排依据与引用同步清单在合并处留注记。**推广(2026-07-31,GH #85 check LOW-1 沉淀):** 同纪律从编号重排推广到一切构成/位置/命名变更——回写规范时必须 grep 旧定位词全仓核对交叉引用(首例:GH #85「本组」指标从组头行下移组条行后,ui-guidelines 三处 + surface brief 一处「组头/标头『均延』」旧定位词残留,check 抓为 LOW)。「全仓」核对含三处规范正文(DESIGN.md / surface briefs / ui-guidelines)——移除/迁移/**删除**具名消费方(类名/组件名)时 grep 该名称于三处规范,并核对 surface brief related_targets 的实体存活性(文件已删而清单残留即失实;第二例:GH #136 UptimeMicroStrip 删除后清单残留,check 沉淀)(首例:GH #90,同日新登的 mono 字族消费方指针 AppHeader .version 当日被迁移失效,check MEDIUM-1)。
