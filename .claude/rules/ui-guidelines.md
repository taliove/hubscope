...
- **UptimeStrip 是组级 24h 分段可用率条的唯一组件**(GH #64 登记,spec 0017 ticket 1;2026-07-30 设计评审;2026-07-31 GH #85 细带化 + 指标下移同行修订;2026-07-31 GH #87 定宽 360px 修订;2026-07-31 GH #88 对齐卡内条规格修订):**位置与构成(2026-07-31 GH #88 修订,取代 GH #87 的 360px/6px 细带登记)**——状态板分组 section 的第二行 = **组条(左对齐定宽 246px,格约 8.3×10px,与卡内条逐像素同规格)+「本组:24h 可用率 X · 均延 Y」指标右对齐同行(指标 margin-left:auto,条与指标之间自然留白)**,位于组头行下方、卡片矩阵上方;**「本组:」聚合指标自组头行移出**(组头行只剩折叠箭头/组名/端点计数/状态计数 chips(+ 协议收敛 tag)/分享按钮)——形态 → 读数一次扫完;「条收进组头行」方案经用户否决(组头左侧内容逐组变宽且随轮询变化,条内联后跨组位置参差、左侧长内容折行风险,跨组对齐是时间轴语言的核心价值,不能牺牲)。**折叠恒显**——收起卡片矩阵后组条行照常渲染,各组健康形态仍可扫读(状态板读者 3 秒场景,spec 0017 user story 3)。形态 = §2 分段填满式(24 格 flex 槽、**格高 10px 与卡内条同高**(2026-07-31 GH #88;**GH #85「组条 6px、卡内 10px 分档」登记随本票显式撤销**——分档诊断错误:实机三轮证据表明丑的根因是格比例(全宽时 ≈46px 长胶囊、360px 时 13×6px 扁丸,都读作「灯/药丸」),非格高度)、`--hs-radius-xs`、2px 间距);**组级条无 26px 行内标签、无 LatencySparkline 对齐约束**——其 2px 间距是形态沿用,不是 §5 LatencySparkline 条目的 GAP=2px 共享常量,不受「改动必须同步」纪律管辖。**对齐约束(2026-07-31 GH #88 定稿,取代 GH #87 的 360px 定宽):条宽定值 246px——格宽算术 (246 − 23×2) / 24 ≈ 8.3px,与 272px 卡片扣除 26px 行内标签后的条槽同宽,组条格与卡内格逐像素同规格(近方形小点);跨组条完全同宽、左缘严格对齐,右缘亦因定宽天然对齐。窄视口下条可收缩(flex-shrink 1 + min-width 0),与指标 nowrap 叠加不撑出横向滚动(§4)。跨层级一致性(本票核心收益,用户裁选定稿 2026-07-31):组条与卡内条同格形同色,「组的条 = 卡片的条的聚合」读感成立;GH #85/GH #87 两轮(6px 细带、360px 定宽)只压单维度,治不了格比例,定稿直接对齐卡内条规格。**着色 = §3 批 59 三档(≥95% success / <95% warning / 0% 有探测全失败 danger / 无探测 `--hs-border` 灰),tooltip 与 EndpointCard dots 同构「HH:mm 时段 · 成功 X/N」,无探测桶「HH:mm 时段 · 无数据」;空组/全禁用聚合无 bucket_start,tooltip 省略时间前缀裸显「无数据」,24 格全灰——**永不读作「全部正常」**(批 56 空态纪律在组级条的镜像)。
