...
- **StatusBadge 词随灯着色(2026-07-30,GH #71,shape 定稿):** 状态词颜色 = 状态语义色——warning/danger/failing 用令牌本体(白底 4.92/4.8/5.1 均过 AA),healthy 用 `--hs-success-text`(见上条);色+词双编码从「点色词墨」升级为「点词同色」。词本就是 Badge 的一部分,不构成第二状态灯;降级成因副标签保持 secondary 不随词着色。组件规格(点尺寸 sm 9px / md 11px)见 dashboard surface brief StatusBadge 节。
...
- **StatusBadge 是唯一的状态展示组件**【规格已迁 surface brief(GH #50),保留备查不再更新】(2026-07-30 修订指针:词随灯着色语义见 §3「success 文字/图形分工」与「StatusBadge 词随灯着色」条,组件规格见 dashboard surface brief),需要展示 endpoint 状态处一律复用,禁止第二个状态灯实现。
