...
- **UptimeStrip 是组级 24h 分段可用率条的唯一组件**(GH #64 登记,spec 0017 ticket 1;2026-07-30 设计评审):状态板分组标头(group-header)下方、卡片矩阵上方,**折叠恒显**——收起卡片矩阵后各组健康形态仍可扫读(状态板读者 3 秒场景,spec 0017 user story 3)。形态 = §2 分段填满式(24 格 flex 槽、格高 10px、`--hs-radius-xs`、2px 间距,与 EndpointCard 卡内条同规格);**组级条无 26px 行内标签、无 LatencySparkline 对齐约束**——其 2px 间距是形态沿用,不是 §5 LatencySparkline 条目的 GAP=2px 共享常量,不受「改动必须同步」纪律管辖。着色 = §3 批 59 三档(≥95% success / <95% warning / 0% 有探测全失败 danger / 无探测 `--hs-border` 灰),tooltip 与 EndpointCard dots 同构「HH:mm 时段 · 成功 X/N」,无探测桶「HH:mm 时段 · 无数据」;空组/全禁用聚合无 bucket_start,tooltip 省略时间前缀裸显「无数据」,24 格全灰——**永不读作「全部正常」**(批 56 空态纪律在组级条的镜像)。
