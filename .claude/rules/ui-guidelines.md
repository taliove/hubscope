...
- **UptimeStrip 是组级 24h 分段可用率条的唯一组件**(GH #64 登记,spec 0017 ticket 1;2026-07-30 设计评审;2026-07-31 GH #85 细带化 + 指标下移同行修订):**位置与构成(2026-07-31 修订,取代「分组标头下方独占一行全宽」)**——状态板分组 section 的第二行 = **细带(flex-1)+「本组:24h 可用率 X · 均延 Y」指标右对齐同行**,位于组头行下方、卡片矩阵上方;**「本组:」聚合指标自组头行移出**(组头行只剩折叠箭头/组名/端点计数/状态计数 chips(+ 协议收敛 tag)/分享按钮)——形态 → 读数一次扫完;「条收进组头行」方案经用户否决(组头左侧内容逐组变宽且随轮询变化,条内联后跨组位置参差、左侧长内容折行风险,跨组对齐是时间轴语言的核心价值,不能牺牲)。**折叠恒显**——收起卡片矩阵后组条行照常渲染,各组健康形态仍可扫读(状态板读者 3 秒场景,spec 0017 user story 3)。形态 = §2 分段填满式(24 格 flex 槽、**格高 6px**(2026-07-31 GH #85 从 10px 细带化,用户实机反馈「格子太大」;组级条是全组扫读带,EndpointCard 卡内条 10px 端点粒度证据不动,两处格高自此分档)、`--hs-radius-xs`、2px 间距);**组级条无 26px 行内标签、无 LatencySparkline 对齐约束**——其 2px 间距是形态沿用,不是 §5 LatencySparkline 条目的 GAP=2px 共享常量,不受「改动必须同步」纪律管辖。**对齐约束(GH #85 登记):条左缘全组严格对齐;右缘随指标文字长度有几 px 参差(格宽差异 <2%,不可察),登记为已知并接受——禁止为右缘对齐把指标文字定宽。**着色 = §3 批 59 三档(≥95% success / <95% warning / 0% 有探测全失败 danger / 无探测 `--hs-border` 灰),tooltip 与 EndpointCard dots 同构「HH:mm 时段 · 成功 X/N」,无探测桶「HH:mm 时段 · 无数据」;空组/全禁用聚合无 bucket_start,tooltip 省略时间前缀裸显「无数据」,24 格全灰——**永不读作「全部正常」**(批 56 空态纪律在组级条的镜像)。
  - **聚合口径(防作假硬约束):** 组内 **enabled** entries 的 dots_24h 按小时求和 total/failures(探测加权,与后端 `internal/server/overview.go` groupAccumulator 同定义,无筛选时与标头标量构造性相等),**禁止按端点简单平均**——低流量端点不得掩盖高流量中断;禁用端点的陈旧探测不染色组条(批 59「卡片所有数字一律从快照 enabled entries 计算」同口径)。聚合桶 p50_ms 恒 null——跨端点 P50 无法由分桶百分位推导,组条不喂 sparkline。
  ...
  - **筛选口径(登记为已知并接受,批 59「卡片内部自洽优先于与页面逐字相等」先例):** 组条随页面筛选与分组维度(厂商/能力/协议)重算——数据源与卡片矩阵是同一份筛选后 entries,条与矩阵构造性永不矛盾;分组标头的「24h 可用率 / 均延」标量仍为后端未筛选口径,有筛选时两者可能数值不一致,组条与矩阵自洽优先于与标头逐字相等(与批 59 分组卡均延口径同例)。
