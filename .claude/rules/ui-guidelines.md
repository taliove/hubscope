...
- **UptimeStrip 是组级 24h 分段可用率条的唯一组件**(GH #64 登记,spec 0017 ticket 1;2026-07-30 设计评审;2026-07-31 GH #85 细带化 + 指标下移同行修订;2026-07-31 GH #87 定宽 360px 修订):**位置与构成(2026-07-31 GH #87 修订,取代 GH #85 的 flex-1 全宽登记)**——状态板分组 section 的第二行 = **细带(左对齐定宽 360px,格约 13×6px)+「本组:24h 可用率 X · 均延 Y」指标右对齐同行(指标 margin-left:auto,条与指标之间自然留白)**,位于组头行下方、卡片矩阵上方;**「本组:」聚合指标自组头行移出**(组头行只剩折叠箭头/组名/端点计数/状态计数 chips(+ 协议收敛 tag)/分享按钮)——形态 → 读数一次扫完;「条收进组头行」方案经用户否决(组头左侧内容逐组变宽且随轮询变化,条内联后跨组位置参差、左侧长内容折行风险,跨组对齐是时间轴语言的核心价值,不能牺牲)。**折叠恒显**——收起卡片矩阵后组条行照常渲染,各组健康形态仍可扫读(状态板读者 3 秒场景,spec 0017 user story 3)。形态 = §2 分段填满式(24 格 flex 槽、**格高 6px**(2026-07-31 GH #85 从 10px 细带化,用户实机反馈「格子太大」;组级条是全组扫读带,EndpointCard 卡内条 10px 端点粒度证据不动,两处格高自此分档)、`--hs-radius-xs`、2px 间距);**组级条无 26px 行内标签、无 LatencySparkline 对齐约束**——其 2px 间距是形态沿用,不是 §5 LatencySparkline 条目的 GAP=2px 共享常量,不受「改动必须同步」纪律管辖。**对齐约束(2026-07-31 GH #87 定稿,取代 GH #85 右缘参差登记):条宽定值 360px——跨组条完全同宽、左缘严格对齐,右缘亦因定宽天然对齐;GH #85「右缘随指标文字长度几 px 参差,登记为已知并接受」随定宽自然失效撤销。窄视口下条可收缩(flex-shrink 1 + min-width 0),与指标 nowrap 叠加不撑出横向滚动(§4)。定宽理由(用户实机裁决 2026-07-31):GH #85 细带化后格高 6px 方向对,但全宽 24 格平分下内容宽 ≈1150px 时每格 ≈76px,一串长胶囊远看仍是一排大灯——格宽 = 条宽 ÷ 24,只压高度治不了「灯太大」,必须收敛条总宽;定宽 360px 后格约 13×6px,「灯」感消失、读作微缩时间轴。**着色 = §3 批 59 三档(≥95% success / <95% warning / 0% 有探测全失败 danger / 无探测 `--hs-border` 灰),tooltip 与 EndpointCard dots 同构「HH:mm 时段 · 成功 X/N」,无探测桶「HH:mm 时段 · 无数据」;空组/全禁用聚合无 bucket_start,tooltip 省略时间前缀裸显「无数据」,24 格全灰——**永不读作「全部正常」**(批 56 空态纪律在组级条的镜像)。
