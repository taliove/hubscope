...
2. **测试门禁(仅 dev)**: `make backend-test` + `make frontend-test`(后端测试 + 前端 test/typecheck/build),不绿不部署。**刻意不含 install-test**:它验证的是生产安装脚本 scripts/install.sh(非 docker 部署路径),且两个用例假设干净机器(无 hubscope 用户、go 不在 /usr/bin),在测试服务器本体上恒失败;tag 部署依赖发版时 CI 已绿
3. **数据备份**: 备份数据库并记录回滚信息;同时记录 DB_FRESH(部署前是否存在数据库)
4. **镜像构建**: 根据模式构建 Docker 镜像;dev 部署每次传入新鲜时间戳作 VERSION 与 CACHEBUST,**前后端强制重新编译**,不吃旧缓存层,版本号恒为部署时刻 `dev-YYYYMMDD-HHMMSS`
5. **容器管理**: 停止旧容器、启动新容器
6. **权限修复**: 设置数据目录为 uid 10001
7. **健康检查**: 验证服务可用性（30 次重试，60 秒超时）
8. **账号初始化**: 仅 DB_FRESH=1(首次部署无历史数据)时创建默认管理员账号
9. **部署验证**: 显示版本、容器状态、前端哈希、数据库大小
10. **镜像清理**: 自动删除超过 7 天的旧镜像（保留最近 3 个版本）
...
6. **首次部署初始化**: 仅在 DB_FRESH=1(部署前不存在 app.db)时创建默认超管(admin/HubScope2026!),已有数据绝不动;禁止用「尝试创建账号并捕获报错」做探测——在 `set -o pipefail` 下该管道恒失败,且探测命令本身有副作用(会真建一个弱密码账号)。

7. **缓存与时间戳**: Dockerfile 内 `RUN date` 生成的时间戳会随缓存层复用而陈旧;必须在构建命令传 `--build-arg VERSION=`(Go 层)与 `--build-arg CACHEBUST=`(前端层),保证每次部署前后端都重新编译、版本号恒为部署时刻。

8. **数据迁移安全**: SQLite 迁移失败不破坏数据(事务回滚),但容器启动失败;始终保留最近 3 个备份。

9. **健康检查必要性**: 不能仅依赖 `docker ps` STATUS,必须验证 `/healthz` 返回 200;容器可能运行但服务异常。

10. **install-test 不属于本门禁**: scripts/install_test.sh 的 `missing go` 用例假设 go 不在 /usr/bin(本机 go-1.26 恰好装在 /usr/bin,悬空符号链接遮蔽不住),`useradd` 用例假设机器无 hubscope 用户(测试服务器本体装过,uid=997 已存在)——两个用例在本机恒失败,应在干净 CI 环境跑;其修复另立 ticket,不堵测试线部署。
