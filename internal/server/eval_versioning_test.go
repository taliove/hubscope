...
func TestPreV3UpgradePurgesLegacySuites(t *testing.T) {
	...
	if len(suites) != 7 {
		t.Fatalf("suites after upgrade = %d, want 5 capability suites + 2 benchmark disabled: %v", len(suites), suites)
	}
	for _, s := range suites {
		...
		if s["key"] == "mmlu" || s["key"] == "ifeval" {
			...
			// from the purge as in-bank suites; their 100-case frozen
			// subsets are covered by the benchmark seed tests.
			continue
		}
	...
	if len(again) != 7 {
		t.Fatalf("suites after reopen = %d, want still 7 (5 capability + 2 benchmark exempt)", len(again))
	}
	for _, s := range again {
		for _, key := range legacyKeys {
			if s["key"] == key {
