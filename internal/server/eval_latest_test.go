...
func TestEvalLatestScores(t *testing.T) {
	...
	// Round 1: both models over the basic suite.
	run1 := triggerEval(t, ts.URL, basicID, smartID, dumbID)
	...
	smart := findLatest(t, rows, "basic", "smart-model")
	if smart["score"] != 1.0 {
		t.Errorf("smart basic score = %v, want 1", smart["score"])
	}
	if int64(smart["eval_run_id"].(float64)) != run1 {
		t.Errorf("smart basic run = %v, want %d", smart["eval_run_id"], run1)
	}
	...
	dumb := findLatest(t, rows, "basic", "dumb-model")
	if dumb["score"] != 0.0 {
		t.Errorf("dumb basic score = %v, want 0", dumb["score"])
	}

	// Round 2: only the smart model, over the same suite. The dumb model's
	// latest row must keep pointing at round 1.
	run2 := triggerEval(t, ts.URL, basicID, smartID)
	...
	if int64(smart["eval_run_id"].(float64)) != run2 {
		t.Errorf("smart basic run = %v, want %d", smart["eval_run_id"], run2)
	}
	dumb = findLatest(t, rows, "basic", "dumb-model")
	if int64(dumb["eval_run_id"].(float64)) != run1 {
		t.Errorf("dumb basic run = %v, want %d (untouched by run 2)", dumb["eval_run_id"], run1)
	}

	// A run over a second suite adds its pairs without disturbing the first.
	reasoningID := suiteIDByKey(t, ts.URL, "reasoning")
	...
	reasoning := findLatest(t, rows, "reasoning", "smart-model")
	if reasoning["score"] != 1.0 {
		t.Errorf("smart reasoning score = %v, want 1", reasoning["score"])
	}
	if int64(reasoning["suite_id"].(float64)) != reasoningID {
		t.Errorf("reasoning suite_id = %v, want %d", reasoning["suite_id"], reasoningID)
	}
}
