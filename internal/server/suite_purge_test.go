...
func TestSuitePurgeCascade(t *testing.T) {
	...
	if len(suites) != 9 {
		t.Fatalf("fresh bank suites = %d, want 5 capability suites + 4 benchmark disabled: %v", len(suites), suites)
	}
	for _, s := range suites {
		if s["capability"] == "" {
			t.Errorf("legacy suite %v listed on a fresh bank, want purged/never seeded", s["key"])
		}
		if s["key"] == "mmlu" || s["key"] == "gsm8k" || s["key"] == "agieval_zh" || s["key"] == "cruxeval" {
			if s["enabled"] != false {
	...
	if len(after) != 8 {
		t.Fatalf("suites after purge = %d, want 8 (4 capability + 4 benchmark exempt): %v", len(after), after)
	}
	for _, s := range after {
	...
	if len(again) != 8 {
		t.Errorf("suites after third boot = %d, want still 8 (4 capability + 4 benchmark exempt)", len(again))
	}
	for _, s := range again {
