...
func TestSuitePurgeCascade(t *testing.T) {
	...
	// benchmark suites (ADR 0013), which seed disabled by design until
	// the ticket-99 cutover and are exempt from the purge as in-bank suites —
	// no pre-v3 legacy suite (capability "") may ever be listed.
	ts, db := openSuitesServer(t, dbPath)
	suites := fetchSuites(t, ts.URL, "")
	if len(suites) != 7 {
		t.Fatalf("fresh bank suites = %d, want 5 capability suites + 2 benchmark disabled: %v", len(suites), suites)
	}
	for _, s := range suites {
		...
		if s["key"] == "mmlu" || s["key"] == "cruxeval" {
			if s["enabled"] != false {
				t.Errorf("%v benchmark suite enabled = %v, want false until cutover", s["key"], s["enabled"])
			}
			continue
		}
		if s["enabled"] != true {
	...
	if len(after) != 6 {
		t.Fatalf("suites after purge = %d, want 6 (4 capability + 2 benchmark exempt): %v", len(after), after)
	}
	for _, s := range after {
	...
	if len(again) != 6 {
		t.Errorf("suites after third boot = %d, want still 6 (4 capability + 2 benchmark exempt)", len(again))
	}
	for _, s := range again {
